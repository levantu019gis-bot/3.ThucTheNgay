...
- **Template metadata** — JSON mô tả PPTX template riêng của target, slide mẫu, map frame và placeholders.
...
Renderer nhận composition, target config, template metadata và output size để tạo map image.

**Consequences:**
- Render dùng `view.center`, `view.scale`, visible layer order, grid settings, background RGB và map frame aspect ratio; geographic read window được suy ra từ center, mẫu số scale, và kích thước vật lý của map frame trong template metadata.
...
- Final render đọc raster ở chất lượng phù hợp output size chính thức từ template metadata.
...
#### FR-20: Load target-specific template metadata

Mỗi target khai báo `export.template_metadata_file`; metadata trỏ tới PPTX template riêng của target.

**Consequences:**
- Metadata chứa `template_pptx`, `slide_index`, map frame và placeholders.
- Placeholder lookup dùng shape `name` là chính và `fallback_id` tùy chọn.
- Validation báo lỗi nếu template metadata, template PPTX, slide hoặc required placeholders thiếu.
- Minimum compatibility checks for MVP: template PPTX exists, slide index resolves, required shape names or fallback ids resolve, map frame dimensions are positive, and image/text replacement can be completed on a dry-run copy; failure is an `error` blocking export.
...
**Consequences:**
- Mỗi exported composition copy sample slide từ target-specific template.
- Các target template trong MVP phải dùng cùng base/theme/master tương thích.
- App thay map image và text placeholders theo template metadata.
...
- Missing required template fields or unresolved placeholders in `txt_line_template` are validation `error`; optional fields may render as empty only when explicitly marked optional in config/template metadata.
...
- Không analyze PPTX template trực tiếp trong app chính; script riêng tạo template metadata trước.
...
- Target-specific PPTX template metadata.
...
- **SM-5:** Preview and final render match in center, scale, visible layer order, grid interval and map frame aspect ratio; final PNG size is recorded from template metadata/export settings. Pixel-level tolerance is calibrated with real sample imagery. Validates FR-13, FR-15, FR-16.
...
2. **Architecture blocker:** What exact PPTX template metadata schema will the separate analyzer script produce?
